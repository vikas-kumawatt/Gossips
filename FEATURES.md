...
**Per-address caps.** Two budgets bound what one address has in flight, both read from a single snapshot by `censusForAddress()`: `MAX_PENDING_PER_EMAIL` (5) on live rows, and `MAX_SENDS_PER_EMAIL` (15) on emails actually sent, summed from `resendCount` across those rows. Exceeding either is a 429 with `Retry-After` set to the wait until the oldest row expires — rejected, never evicting an existing row, so an attacker cycling a victim's inbox can neither keep going indefinitely nor invalidate a code the victim is holding. The row cap is enforced *after* the insert rather than before it: the pre-insert count is only a fast path, because a count-then-write with a bcrypt hash in the window let simultaneous requests all pass it. The post-insert pass re-counts, ranks by `_id`, and deletes only its own row if it is the excess one, so concurrent racers agree on who survives without evicting anybody. Ranking by `_id` and not `expiresAt` is deliberate — a resend rewrites the latter. A residual window of one round trip remains by design; closing it would need reusable `(email, slot)` rows, and reusing a row breaks the ticket-to-row binding that keeps one person's code from applying another's password.

**Email delivery failure.** If Brevo rejects the send, the row is deleted rather than left holding a code nobody received, and the response is a 502 with `Retry-After: 5` and `retryable: true`.

**Verification ticket lifetime.** The ticket is signed for `OTP_TTL_MS × OTP_MAX_SENDS` (50 minutes) — the longest a signup can legitimately stay open, not the life of one code. The row's `expiresAt` is the real deadline and both `verifyOtp` and `resendOtp` check it, answering 410 with guidance; the ticket's own `exp` is only a backstop for a token whose row is already gone. Scoping it to a single OTP window instead meant that when a resend's response failed to reach the client, the client's ticket expired while its row was still alive, discarding a live signup with a valid code sitting in the user's inbox.
...
**How it works:** `POST /auth/resend-otp` → `reissueOtp()` with a **60-second cooldown** and a cap of **5 sends** per pending row (`OTP_MAX_SENDS`); resending deliberately does not reset the guess counter, or the guess budget would become `OTP_MAX_ATTEMPTS × OTP_MAX_SENDS` with the attacker choosing when to resend. Verification allows **5 wrong guesses** (`OTP_MAX_ATTEMPTS`), decremented atomically inside a `findOneAndUpdate` filter so concurrent guesses can't both spend the same attempt. `reissueOtp` also refuses when the per-address send budget is spent (`MAX_SENDS_PER_EMAIL`, see above) — the per-row counters cannot see an address's other four rows, and five rows resent to their caps is 25 messages to one inbox. Per-IP limiters sit on top (verify 30/15min, resend 10/hour).

**Failed delivery rolls the row back.** The new code is written before the mail is sent, so a delivery failure restores the previous `codeHash`, `lastSentAt`, `expiresAt` and `resendCount`, filtered on the hash just written so a concurrent resend that *did* deliver wins. Without the rollback a transient Brevo outage left the row holding a code nobody received while invalidating the one already in the user's inbox, having also spent a send and reset the cooldown. The response is a 502 with `Retry-After`.

**Dead ends are dead ends.** Once the guess budget is spent, a resend cannot refill it, so `reissueOtp` refuses outright rather than mailing a code that could never be entered, and both the verify and resend responses say "please start over" and set `locked: true` — the wording used to say "request a new one", which was a loop with no exit. `VerifyOtpPage.jsx` renders `locked`/`exhausted` as a terminal state: input disabled, **Start over** promoted to the primary button, Verify and Resend removed rather than left live and useless.

**Client countdowns.** Two independent clocks: `sentAt` drives the code-expiry countdown, `resendReadyAt` drives the resend button. They were one value, corrected by rewinding `sentAt` on a 429 — which was fine while every `retryAfter` was the 60-second cooldown, but the per-address cap waits for a row to expire, and rewinding by minutes would have claimed the code had far more life than it did. A zero code-expiry countdown is treated as a hint rather than a verdict, since only the server knows the row's real `expiresAt`: both actions stay live, and a resend either renews a row that was alive all along (recovering a client whose earlier resend response was lost) or returns the 410 that ends the flow properly.

### Email/password login

**What it does:** Authenticates with username-or-email plus password.

**How it works:** `POST /auth/login` → `loginUser` looks the user up with `{...HUMAN_ACCOUNT}` (bot rows excluded by construction), rejects Google-only accounts with `needPasswordSetup: true`, compares via bcrypt, clears any lockout state, records sign-in country asynchronously, and issues tokens. Rate-limited 10/15min/IP.

**Per-account lockout.** 5 consecutive failed attempts (`MAX_FAILED_LOGIN_ATTEMPTS`) lock the account for 15 minutes (`ACCOUNT_LOCKOUT_DURATION_MS`), answered as a 429 with `Retry-After` and `locked: true`; a password reset clears `failedLoginAttempts`/`lockoutUntil` as an unlock path. The counter is per account rather than per IP, which is what a distributed credential-stuffing run rotating addresses would otherwise sidestep. Three helpers — `rejectIfLockedOut`, `rejectFailedAttempt`, `passesTwoFactor` — are shared by both sign-in paths so the lockout also covers Google sign-in and the second-factor step, each returning a boolean meaning "already answered, return now". The counter reset happens *after* the second factor, not on password success: resetting it earlier handed the code step a budget that was always zero.

**Two-factor gate.** When `twoFactorEnabled`, a first request with no code gets `200 { needTwoFactor: true }` and no token; the client re-posts the same body plus `twoFactorCode`, accepted as either a 6-digit TOTP (±1 step drift) or an 8-character single-use backup code. A wrong code spends an attempt from the same lockout counter a wrong password does — otherwise a known password plus an unmetered code prompt is 10⁶ guesses against six digits. `UserAuthForm.jsx` renders the code step in place of the login form and carries the originating route, since both sign-in paths can demand a code.

**Trusted devices.** `UserSession.isTrusted` / `trustedAt` / `trustedUntil`, managed only by `utils/trustedDevices.js`. A device becomes trusted when a two-factor challenge is passed *and* the person ticks "Trust this device for 30 days" (`rememberDevice`, off by default); for `TRUSTED_DEVICE_DURATION_MS` afterwards `loginUser` and `googleLogin` skip the challenge. Trust cannot be granted on an account without 2FA — there would be nothing to skip, and banked trust would then be honoured the moment it was switched on. It is withdrawn by revoking the session (Active Sessions, "log out other devices", or a password reset, all of which delete the row), by the window lapsing, or by `untrustAllDevices()` when 2FA is enabled or disabled — so turning 2FA off and back on after a compromise cannot leave devices trusted under the old secret. `GET /auth/sessions` applies the expiry when reporting, and `ActiveSessionsModal.jsx` shows a **Trusted** badge with the date it lapses. These fields previously defaulted to `true` and were re-set on every token issue, so every session read as trusted, nothing could be untrusted, and `trustedAt` recorded the last refresh — on a screen whose whole purpose is helping someone judge whether a device is theirs.
...
**Google is the only provider.** The unused `appleId`/`facebookId` fields are gone from `User`. Their MongoDB indexes outlive the schema, so `scripts/auditUserIndexes.js` classifies any index whose field is absent from the schema as droppable — `npm run users:index-audit` reports, `users:index-audit:apply` drops — with `botUserIndexAudit.test.js` pinning `appleId_1`/`facebookId_1`/`githubId_1` as expected candidates.

**Same gate as the password path.** `googleLogin` runs the lockout check, two-factor gate and trusted-device skip that `loginUser` does, positioned before its `User.updateOne` so a refused attempt persists nothing. It previously issued a session on a Google token alone, so any account with 2FA on that also had a linked Google address could be signed into with the first factor only.

**Firebase configuration.** Credentials resolve through `utils/firebaseAdmin.js`, shared by Google sign-in and push notifications, accepting either the decomposed `FIREBASE_PROJECT_ID`/`FIREBASE_PRIVATE_KEY`/`FIREBASE_CLIENT_EMAIL` trio (with `\n` un-escaped, since env-var UIs store the key that way) or `FIREBASE_SERVICE_ACCOUNT` as inline JSON or a file path. Missing or unparseable credentials produce a startup warning naming every accepted shape rather than crashing, and `POST /auth/googlelogin` answers HTTP 503. The two consumers used to decide this independently, and auth's copy understood only the decomposed trio — so a deployment credentialed with `FIREBASE_SERVICE_ACCOUNT` had working push notifications and a Google sign-in permanently reporting itself unconfigured. Readiness is now checked per request via `admin.app()` rather than read off a module-load flag, because push initialises lazily and may have created the default app after this module loaded. On the client, `isFirebaseConfigured` (from the `VITE_FIREBASE_*` vars) both refuses `authWithGoogle` early and hides the "Continue with Google" button entirely, so an unconfigured deployment doesn't offer a control whose only behaviour is an error toast; `getAuth()` is called on demand rather than at module scope, since it throws `auth/invalid-api-key` and would otherwise blank the whole app on one missing variable. The two sides judge configuration from different env vars and a deployment can have one without the other, so `UserAuthForm` also hides the button after a 503 from `/auth/googlelogin` — learned from the failure rather than from a config endpoint, which would make every visitor wait on a request to spare a misconfigured deployment one wasted popup. It isn't persisted, so correcting the server config and reloading restores the button.

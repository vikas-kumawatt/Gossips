...
import VoiceComposerBar from "../components/Chat/VoiceComposerBar";
// Still needed directly for the in-flight upload preview, which sits outside the
// grouped list because it has no id to group or key on yet.
...
/*
 * The attachment rules — size cap, accepted types, the `accept` attribute and the
 * per-message limit — moved to lib/composerMedia.js, because the group composer had its
 * own narrower copy of all four and the server has only one opinion about any of them.
 */
...
/*
 * `formatInstagramTimestamp`, `messagePreviewLabel`, `getMessageBody` and
 * `MessageBubble` now live in lib/chatMessage.js and components/Chat/MessageBubble.jsx,
...
const UserConversationPage = ({
   ...
   *
   * `useMediaTray` owns the part that was subtle enough to be worth having exactly one
   * of: the URL registry, the per-reason rejection tally, and putting a selection back
   * after a failed send without leaking what was already there.
   */
  const tray = useMediaTray({ onReject: (message) => toast.error(message) });
  const selectedMediaFiles = tray.items;
  /*
  ...
  /*
   * `voiceStaticWaveform` is the only piece of recorder state left here: it is the
   * flat placeholder the preview bar falls back to when a clip somehow carries no
   * envelope. Everything else moved into useVoiceRecorder.
   *
   ...
   *
   * The microphone, the AudioContext, the animation frame and the recording timers all
   * used to be cleaned up here; they belong to `useVoiceRecorder` now and it has its
   * own unmount effect, so the release happens wherever the hook is used rather than
   * only on the one page that remembered to do it.
   */
  useEffect(
    () => () => {
  ...
  const validateMessage = (media = []) => {
    if (!newMessage.trim() && selectedMediaFiles.length === 0 && media.length === 0) {
  ...
  const handleSendMedia = async () => {
    ...
    // Capture files and show optimistic bubble immediately. `take` empties the tray and
    // hands the items over without revoking their URLs — the optimistic bubble below is
    // still rendering from them.
  ...
  /*
   * Validation and the rejection tally moved to lib/composerMedia.js and useMediaTray.
   * The list of accepted types here was a third copy of the upload rules and narrower
   * than both the others, which is what rejected every photo taken on an iPhone (#110).
   */


  /*
   * Recording lives in useVoiceRecorder now, and the bars in VoiceComposerBar.
   *
   * ~200 lines of MediaRecorder, AnalyserNode and playback state used to sit here,
   * which is exactly why the group composer's microphone button recorded nothing.
   * `sendVoiceNote` below stays: uploading and the optimistic bubble are this page's
   * business, and the two threads send differently.
   */
  ...
  const sendVoiceNote = async () => {
    ...
    /*
     * `takePreview` clears the composer and hands the clip over in one step, and
     * deliberately does *not* revoke the blob URL — the optimistic bubble below uses it
     * as its audio source while the upload is in flight, so it has to outlive this
     * call. Releasing it is the `finally` block's job.
     */
    const clip = voice.takePreview();
    if (!clip) return;
    const { file, duration, url: blobUrl, waveformSnapshot } = clip;
    const tempId = `temp-${Date.now()}-${Math.random()}`;
  ...
  /*
   * Grouping and dividers come from lib/chatMessage now — the group thread had no
   * dividers and no bubble stacking at all, because this logic lived in this file.
   */
  ...
  /*
   * The reply strip, the edit strip and the attachment tray moved into
   * components/Chat/ChatComposer.jsx. The group composer had its own reply strip that
   * printed `replyingTo.content` raw — blank for a photo or a voice note — and no edit
   * strip or tray at all.
   */
      ...
      */}
          ...
          <div className="min-h-full flex flex-col">
            ...
            <div className="pb-4">
              <MessageList
                ...
                /* Always the peer in a DM — there is only one other person, so the
                   sender of any incoming message is by definition them. */
                avatarFor={() => ({
                  src: headerUser?.profilePic,
      ...
      <div className="shrink-0 bg-black" style={{ paddingBottom: "env(safe-area-inset-bottom)" }}>
        {readOnly ? (
          ...
          /*
           * No poll and no document button here. A poll between two people collapses into
           * a question you could just ask, and the anonymous-vote setting is meaningless
           * at n=1 — whoever answers is identifiable by elimination. GroupChatPage passes
           * both. PollBubble stays wired up below so DM polls created before this still
           * render and remain votable.
           */
          <ChatComposer
            value={newMessage}
            ...
            voice={voice}
            idleWaveform={voiceStaticWaveform}
